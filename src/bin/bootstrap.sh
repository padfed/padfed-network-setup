...
printHelp() {
    echo "Usage: bootstrap.sh version [options]"
#   echo "Usage: bootstrap.sh [version [ca_version [thirdparty_version]]] [options]"
    ...
    echo
    echo "e.g. bootstrap.sh 1.4.3 -s -d"
    echo "would download docker binaries for version 1.4.3"
}
...
binaryIncrementalDownload() {
    ...
    if [ -z "$rc" ] || [ $rc -eq 33 ] || [ $rc -eq 2 ]; then
        ...
        if [ "$localMd5sum" == "$remoteMd5sum" ]; then
            ...
            echo "==> Binary incremental download done."
