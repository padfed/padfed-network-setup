...
function check_org_value() {
   ...
   if   [[ -f $string_file_or_dir && -s $string_file_or_dir ]]; then
        ...
        local l_value=$( cat "$string_file_or_dir" )
        ...
        local l_value=$(configtxgen -configPath "$string_file_or_dir" -printOrg "$MSPID")     
   else
        local l_value="$string_file_or_dir"
   fi

   local l_jpath=".values.MSP.value.config.fabric_node_ous.client_ou_identifier.organizational_unit_identifier"
   local l_found=$( echo "$l_value" | jq -r "$l_jpath" )
   local l_want="client" 
   ...
   local l_jpath=".values.MSP.value.config.fabric_node_ous.admin_ou_identifier.organizational_unit_identifier"
   local l_found=$( echo "$l_value" | jq -r "$l_jpath" )
   local l_want="admin" 
   if [[ $l_want != "$l_found" ]]; then
   ...
   local l_jpath=".values.MSP.value.config.name"
   local l_found=$( echo "$l_value" | jq -r "$l_jpath" ) 
...
function check_consenter_json() {
   ...
   if [[ -s $string_or_file ]]; then
      ...
      local l_value=$( cat "$string_or_file" )
   else
      local l_value="$string_or_file"
   fi
   local l_client_tls_cert=$( echo "$l_value" | jq -r ".client_tls_cert" )
   local l_server_tls_cert=$( echo "$l_value" | jq -r ".server_tls_cert" )
...
function check_single_b64_x509_crt() {
   local l_name="$1"
   local l_value="$2"
   local l_tmpfile=$( mktemp2 "$l_name.crt" )
...
function check_single_b64_crl() {
   local l_name=$1
   local l_value=$2
   local l_tmpfile=$( mktemp2 "$l_name.crl" )
   echo "$l_value" | base64 --decode -i > "$l_tmpfile"
...
function update_channel() {
   ...
   local value=$( cat "$UPDATE_PB.json" )
...
usage() {
  ...
  echo "   options:"
  echo "   -o <original config>: fetch (default)|*.pb (protobuf)|*.block (protobuf)|*.json"
  echo "   -c <channel name>"
  echo "   -m <MSPID>"
...
function main() {
   ...
   *.pb | *.block ) 
            ...
            ;;
   fetch )  readonly CONFIG_PB=$(   mktemp2 "original.config.pb" ) 
            readonly CONFIG_JSON=$( mktemp2 "original.config.pb.json" ) 
            fetch_config "$CONFIG_PB"
            proto_decode_common_block "$CONFIG_PB" "$CONFIG_JSON"
            ;;
   *) echo_red "ERROR: -o [$ORIGINAL_CONFIG] unexpected, must be fetch|*.pb|*.block|*.json" 
      usage
      exit 1
   esac
